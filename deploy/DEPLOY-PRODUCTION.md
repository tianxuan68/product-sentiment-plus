...
```sql
ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'Qwertyuiop@123';
FLUSH PRIVILEGES;
EXIT;
```
...
假设源码在 Windows/本机：`E:\tianxuan\product-sentiment-plus`（Linux 同理改路径）。
...
# 浏览器请求走 Nginx 同源前缀（不要写后端真实端口）
VITE_GLOB_API_URL=/jeecgboot

# 改成你的对外地址（域名或 IP），末尾带 /jeecgboot
...
说明：开发代理是 `/jeecgboot` → `http://127.0.0.1:8005/jeecg-boot`；生产由 Nginx 做同样改写。
...
# 在项目根目录
mkdir -p release
rsync -a --exclude '__pycache__' --exclude '*.pyc' --exclude '.env.local' \
  backend/ release/backend/

# 准备服务器用 .env（不要提交到 git）
cp backend/.env.example release/backend/.env
# 再编辑 release/backend/.env，见下一节模板
```

`release/backend/.env` 模板：

```env
DATABASE_URL=mysql+pymysql://sentiment:DB_PASS@127.0.0.1:3306/jeecg-boot?charset=utf8mb4
# 密码含 @ 必须写成 %40
...
SIGNATURE_SECRET=请改成随机长字符串
```

### 2.4 AI 端打包（代码 + 权重，权重不在 git 里）
...
| `models/bert/common/model/bert_all/` | 情感二分类 | ~0.38GB |

可选：`models/tagging/model/bert_multilabel/`（备用）。
...
rsync -a \
  ...
  product-sentiment-ai/ release/product-sentiment-ai/

# 确认权重在包内（若 .gitignore 导致没进仓库，必须从本机拷）
ls release/product-sentiment-ai/models/tagging/model/bert_hierarchical/hier_config.json
ls release/product-sentiment-ai/models/bert/common/model/bert_all/config.json
```
...
```bash
# Linux / Git Bash
tar -czf sentiment-release.tar.gz -C release .

# 或 Windows PowerShell
# Compress-Archive -Path release\* -DestinationPath sentiment-release.zip
```
...
# 上传（任选）
scp -r release/* root@SERVER_IP:APP_DIR/
# 或
rsync -avz --progress release/ root@SERVER_IP:APP_DIR/
# 或上传 tar 后解压
scp sentiment-release.tar.gz root@SERVER_IP:/tmp/
ssh root@SERVER_IP "mkdir -p APP_DIR && tar -xzf /tmp/sentiment-release.tar.gz -C APP_DIR"
```

前端 dist 单独放 Nginx 目录也可以：

```bash
scp -r front/dist/* root@SERVER_IP:/var/www/sentiment/
```

SQL 若尚未导入：

```bash
scp backend/sql/jeecgboot-slim.sql root@SERVER_IP:/tmp/
ssh root@SERVER_IP "mysql -u sentiment -p'DB_PASS' --max-allowed-packet=512M jeecg-boot < /tmp/jeecgboot-slim.sql"
```
