...
> 本仓库为精简版：后端为 **FastAPI**（`../jeecg-fastapi`），前端为 Vue3。请先启动 FastAPI 后端，再启动本前端项目。
...
- 后端启动见仓库根目录 `README.md` 或 `jeecg-fastapi/README.md`
...
```bash
VITE_PROXY = [["/jeecgboot","http://localhost:8000/jeecg-boot"],["/upload","http://localhost:3300/upload"]]
VITE_GLOB_DOMAIN_URL=http://localhost:8000/jeecg-boot
```

> 说明：把`http://localhost:8000/jeecg-boot` 换成自己地址，其他不用改。
