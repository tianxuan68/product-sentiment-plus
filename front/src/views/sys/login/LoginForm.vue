<template>
  ...
  <Form class="p-4 enter-x" :model="formData" :rules="getFormRules" ref="formRef" v-show="getShow" @keypress.enter="handleLogin">
    ...
    <div v-if="enabledThirdSources.length > 0" class="flex justify-evenly enter-x" :class="`${prefixCls}-sign-in-way`">
      <a v-if="enabledThirdSources.includes('github')" @click="onThirdLogin('github')" title="github"><GithubFilled /></a>
      <a v-if="enabledThirdSources.includes('wechat_enterprise')" @click="onThirdLogin('wechat_enterprise')" title="企业微信"> <icon-font class="item-icon" type="icon-qiyeweixin3" /></a>
      <a v-if="enabledThirdSources.includes('dingtalk')" @click="onThirdLogin('dingtalk')" title="钉钉"><DingtalkCircleFilled /></a>
      <a v-if="enabledThirdSources.includes('wechat_open')" @click="onThirdLogin('wechat_open')" title="微信"><WechatFilled /></a>
    </div>
  </Form>
  <!-- 第三方登录相关弹框 -->
...
<script lang="ts" setup>
  ...
  import { getCodeInfo, getThirdLoginConfig } from '/@/api/sys/user';
  ...
  async function handleLogin() {
    ...
    try {
      ...
      // 密码使用AES加密传输
      const encryptedPassword = encryptPasswordForTransmit(data.password);
