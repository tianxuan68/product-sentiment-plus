...
enum Api {
  ...
  thirdLoginConfig = '/sys/thirdLogin/config',
}
...
export function getUserInfo() {
  return defHttp.get<GetUserInfoModel>({ url: Api.GetUserInfo, timeout: 90 * 1000 }, {}).catch((e) => {
    // Token过期失效，直接跳转登录页面
    ...
    defHttp.post({ url: Api.getCaptcha, params }, { isTransformResponse: false }).then((res) => {
      ...
      if (res.success) {
        if (res.result?.devCode) {
          createErrorModal({
            title: '开发模式验证码',
            content: `未配置短信网关，验证码为：${res.result.devCode}`,
          });
        }
        resolve(true);
      } else {
 ...
 * @param params
 */
export const passwordChange = (params) => defHttp.post({ url: Api.passwordChange, params }, { isTransformResponse: false });
/**
 * @description: 第三方登录
 */
